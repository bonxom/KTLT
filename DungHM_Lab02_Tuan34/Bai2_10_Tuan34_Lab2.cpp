/* Bai 2.10 Tính tích hai ma trận vuông
...
Gợi ý: hãy để ý đến thứ tự truy cập các phần tử trong ma trận, 
tối ưu cache hoặc sử dụng thuật toán tốt hơn  O(N3)
 .
...
Matrix multiply_fast(const Matrix &a_50, const Matrix &b_50) {
    /*****************
    # YOUR CODE HERE #
    Với cách duyệt i-k-j, b.mat[k][j] sẽ duyệt liên tục theo hàng, tận dụng hiệu quả cache → tăng tốc đáng kể
    *****************/
    Matrix c_50;
    for (int i_50 = 0; i_50 < N; ++i_50) {
        for (int k_50 = 0; k_50 < N; ++k_50) {
            unsigned int temp_50 = a_50.mat[i_50][k_50];
            for (int j_50 = 0; j_50 < N; ++j_50) {
                c_50.mat[i_50][j_50] += temp_50 * b_50.mat[k_50][j_50];
            }
        }
    }
    return c_50;
